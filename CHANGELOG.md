...
- **`grantiva console devices get <key-id>`** — a device's stored profile, compliance status with violations, and its recent events. Listing devices is not yet available to API keys; `analytics risk`, `analytics compliance`, and `analytics export --data devices` are the ways to find key IDs for now.
